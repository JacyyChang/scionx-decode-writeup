...
**在一份真實長錄音上發現的注意事項**
（`satnogs_14674078_2026-08-03T07-50-10.ogg`，606 秒）：AFSK 本身是定振幅
...
門檻」：AFSK 本身音調造成的振幅起伏，就算在真封包內部，也會在任何固定比值
上下擺動——用天真的逐區塊門檻去測 `cut_first3.ogg` 三個已知真封包，一個都抓
不到，所以才改用這種匹配濾波的做法（已驗證能在 11 毫秒誤差內找回全部 3 個
已知起點，才敢拿去用在新資料上）。第二階段把每個第一階段存活下來的候選點緊緊
裁切出來，對這一小段真的做 baseline restoration，檢查過零點間隔是不是像真正
AFSK symbol 那樣聚集在 SPS 的倍數上，並拿候選點前面一段長度相同的雜訊窗格當
