# -*- coding: utf-8 -*-
"""
04d_destuff_interactive.py -- extends the mechanism validated in the
hand-built `scionx_destuff_demo.xlsx` (33-bit synthetic example: RawBits
sheet with per-bit ExcludeThisBit/FlipThisBit dropdowns, live-connected to
other sheets) to a REAL raw bit stream: one detected frame of cut_first3.ogg
(default frame 2).
...
Usage: `python 04d_destuff_interactive.py [audio.ogg] [--frame N] [--z-threshold Z]`
Same CLI convention as 04c. Output:
`Output/<audio stem>_frame<N>_destuff_interactive.xlsx`.
"""
...
def write_instructions(wb, frame_no, n_raw, best_offset, n_verifiable_bits, n_telemetry_bits):
    ...
    lines = [
        "SCIONX Destuff 互動練習 -- 真實 Frame 版 v2 (延伸自 scionx_destuff_demo.xlsx 的機制)",
        "",
        ...
        "（原本用整列黃底標 stuff 猜測區段，容易誤會成『這裡已經判定是 stuff bit』，"
        "拿掉了，RawBits 現在沒有任何整列背景色）。",
...
def write_rawbits(wb, decided, ambiguous, seg_label, n_raw, n_fields):
    ...
    red when Yes so those two signals are easy to scan for. Rows are still
    shaded by SegmentGuess (address/data1/zero-run1/data2/zero-run2/FCS/
    trailing), same colors as before, EXCEPT SegmentGuess="stuff" rows get
    no fill at all -- a stuff-labeled row used to get a yellow background
    that looked like a confident "this IS a stuff bit" marker, which
    conflicts with what CandidateStuffPoint (the actual live signal) may be
    saying about that same row; leaving it unfilled avoids that visual
    claim while keeping every other segment's shading for fast scanning."""
    ...
    for i in range(n_raw):
        ...
        if seg_label[i] != "stuff":
            fill = SEG_FILLS.get(seg_label[i])
            if fill:
                for c in range(1, len(headers) + 1):
...
def write_destuffed_hidden(wb, n_ref_bits):
    """Internal join table, hidden by default (see build_workbook): for each
    Position p=1..n_ref_bits (the bit's index in the "stuffing removed"
    sequence), look up which RawBits row currently has that DestuffedSeqNo
    and pull its EffectiveBitValue. This is the ONLY place that re-derives
    "destuffed payload bit p" from RawBits' current E/F choices; CRC_Check,
    Fields, and Beacon Decode all read from here (row for Position p is
    always p+1) rather than re-deriving it themselves, so there's exactly
    one INDEX/MATCH join, not four. A previous version exposed this sheet
    directly (with a reference-comparison mismatch counter) but that's been
    dropped -- see 使用說明's v2 notes; this is now purely plumbing."""
    ws = wb.create_sheet("_DestuffedBits")
    headers = ["Position", "SourceRawBitIndex", "DestuffedBitValue"]
...
def build_workbook(out_path, frame_no, y, yc, start):
    ...
    decided, ambiguous, seg_label = decide_raw_bits(y_at_bit, yc_at_bit, offset_thr, margin, best_offset, n_raw)
    ...
    write_rawbits(wb, decided, ambiguous, seg_label, n_raw, len(fields))
    write_destuffed_hidden(wb, len(REF_BITS))
    write_crc_check(wb)
    write_beacon_decode_sheet(wb, fields, field_row)

    # Tab order: instructions/RawBits/Beacon Decode/CRC_Check up front (what the user
    # actually looks at); the two join/backing sheets pushed to the end and hidden --
    # they're still fully functional (hidden only affects tab visibility), just not
    # meant to be edited (see 使用說明 v2 notes).
    order = ["使用說明", "RawBits", "Beacon Decode", "CRC_Check", "_DestuffedBits", "Fields"]
    wb._sheets = [wb[name] for name in order]
    wb["_DestuffedBits"].sheet_state = "hidden"
...
def main():
    ...
    out_path = os.path.join(OUT_DIR, f"{stem}_frame{args.frame}_destuff_interactive.xlsx")
